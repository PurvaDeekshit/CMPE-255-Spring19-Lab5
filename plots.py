...
def classify_and_plot_grid(cities, k=1):
    ...
    # TODO
    for longi in range (-130, 60):
        for lati in range (20, 55):
            lang_pred = knn_classify(k, cities, [longi, lati])
            plots[lang_pred][0].append(longi)
            plots[lang_pred][1].append(lati)
    ...
    plt.legend(loc=0)  # let matplotlib choose the location
    plt.axis([-130, -60, 20, 55])  # set the axes
    plt.title("{} - Predicted Preferred Language".format(k))
    plt.show()
