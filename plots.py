...
def classify_and_plot_grid(cities, k=1):
    ...
    for longitude in range (-130, 60):
        for latitude in range (20, 55):
            lang_pred = knn_classify(k, cities, [longitude, latitude])
            plots[lang_pred][0].append(longitude)
            plots[lang_pred][1].append(latitude)
    ...
    plt.legend(loc='center left', bbox_to_anchor = (1, 0.5))  # location is center left outside the plot
    plt.show()
...
if __name__ == "__main__":
    ...
    #cities = []
    plot_cities(cities)
    classify_and_plot_grid(cities, 1)
